...
public:
	...
	constexpr explicit Vector( vec_t XYZ ); ///< broadcast initialize
	...
	// copy constructors
	constexpr Vector(const Vector &vOther);
...
inline constexpr Vector::Vector( const Vector &vOther ) : x( vOther.x ), y( vOther.y ), z( vOther.z )
{
	CHECK_VALID( *this );
}
#endif
...
public:
	...
	constexpr Quaternion( vec_t ix, vec_t iy, vec_t iz, vec_t iw ) : x( ix ), y( iy ), z( iz ), w( iw ) {}
...
{
public:
	constexpr RadianEuler( void ) : x( 0 ), y( 0 ), z( 0 ) {}
	constexpr RadianEuler( vec_t X, vec_t Y, vec_t Z ) : x( X ), y( Y ), z( Z ) {}
...
public:
	...
	// Construction/destruction
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
	constexpr QAngle( void );
#else
	QAngle() = default;
#endif
	constexpr QAngle( vec_t X, vec_t Y, vec_t Z );
...
//-----------------------------------------------------------------------------
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
inline constexpr QAngle::QAngle( void )
{
	// Initialize to NAN to catch errors
	x = y = z = VEC_T_NAN;
}
