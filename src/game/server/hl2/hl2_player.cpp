...
#ifdef RTSL
bool CHL2_Player::EquipGGFreezeModule(void)
{
	if ( !m_HL2Local.m_bHasGGFreezeModule )
	{
		m_HL2Local.m_bHasGGFreezeModule = true;
		CPASAttenuationFilter filter( this, "SuitRecharge.Start" );
		EmitSound( filter, entindex(), "SuitRecharge.Start" );

		CSingleUserRecipientFilter user( this );
		user.MakeReliable();

		UserMessageBegin( user, "ItemPickup" );
		WRITE_STRING( "item_physcannon_freeze" );
		MessageEnd();

		CFmtStr hint;
		hint.sprintf( "#RTSL_Hint_GGFreeze" );
...
bool CHL2_Player::RemoveGGFreezeModule( void )
{
	m_HL2Local.m_bHasGGFreezeModule = false;
	return true;
}

bool CHL2_Player::PlayerHasGGFreezeModule( void )
{
	return m_HL2Local.m_bHasGGFreezeModule;
}
#endif
